...
export default function AdminModal() {
  ...
  const handleClose = () => {
    ...
    setErr('');
    setLoading(false);
    setStep('login');       // pour réafficher le login au prochain open
    close();                // ferme la modale via le context
  };

  // Gestion pour l'ouverture modal
  ...
  return (
    <Modal
      ...
      shouldCloseOnEsc
    >
      <button className="adminmodal__close" onClick={handleClose} aria-label="Fermer">
      ...
      {/* -------------------------- DEBUT DU SYSTEME DE PAGINATION INTERNE MODAL ----------------------------- */}
      {step === 'hub' && (
        <section className="adminmodal__panel">
      ...
      {step === 'certifications:new' && (
        <AdminForm
          ...
          mode = "create"
          onCancel={() => setStep('certifs')}
          onSuccess={() => setStep('certifs')}
        />
      )}
      ...
      {step === 'certifications:edit' && editingId && (
        <AdminForm
          ...
          mode = "edit"
          passingId = {editingId}
          onCancel={() => { setEditingId(null); setStep('certifs'); }}
          onSuccess={() => { setEditingId(null); setStep('certifs'); }}
        />
      )}
